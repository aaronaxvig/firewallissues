...
```caveat
(PAN-OS 10.1.14-h9 only) This issue is now resolved. See PAN-OS 10.1.14-h10 Addressed Issues.
```
...
When FIPS mode is enabled on VM-Series firewall in Microsoft Azure environments, HA failover does not trigger the secondary IP address movement.
