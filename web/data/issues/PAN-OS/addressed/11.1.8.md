...
Fixed an issue where the Link LEDs for ethernet1/9 to ethernet1/12 did not turn off after a failover.
...
Fixed an issue where non-tunneled internal GlobalProtect gateway client information was not synced between firewall peers when using a floating IP address.
...
```caveat
VM-Series firewalls across all public and private clouds
```

Fixed an issue where the firewall experienced high dataplane CPU usage when SSL Decryption was enabled.
...
Fixed an issue with an incorrectly open port.
...
Fixed an issue where interfaces running at 10 Gbps did not display the speed and duplex information in the CLI or displayed only as auto.
...
Fixed an issue on the web interface where the DHCPv6 client was not available for VLAN interfaces.
...
Fixed an issue where the firewall had a lower maximum number of Security profiles than expected.
...
Fixed an issue where the firewall did not increment the flow_parse_ip_cksm counter when traffic with an IP address checksum error was received.
